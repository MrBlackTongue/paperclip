...
function writeWorktreeConfigFile(context: WorktreeRuntimeContext, config: PaperclipConfig): boolean {
  const targetPath = context.configPath;
  if (!isWorktreeRepairWriteAllowed(context.worktreeRoot, targetPath)) {
    warnRefusedWorktreeRepairWrite(context, targetPath);
    return false;
  }
  ...
  const source = fs.existsSync(targetPath)
    ? paperclipConfigSchema.parse(JSON.parse(fs.readFileSync(targetPath, "utf8")))
    : null;
  ...
  atomicWriteFile(targetPath, JSON.stringify(nextConfig, null, 2) + "\n");
  return true;
}

function writeWorktreeEnvFile(
  context: WorktreeRuntimeContext,
  previousContents: string | null,
  nextContents: string,
): boolean {
  if (!isWorktreeRepairWriteAllowed(context.worktreeRoot, context.envPath)) {
    warnRefusedWorktreeRepairWrite(context, context.envPath);
    return false;
  }
  return writeEnvFileAtomicallyIfChanged(context.envPath, previousContents, nextContents);
}
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  const repairedEnv = existingContents !== repairedContents
    && writeWorktreeEnvFile(context, existingContents, repairedContents);
