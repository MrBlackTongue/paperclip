...
describeEmbeddedPostgres("issue recovery actions", () => {
  ...
  it("rejects a move to blocked without blockers and keeps the active recovery action", async () => {
    const { companyId, managerId, sourceIssueId } = await seedCompany();
    const action = await seedActiveRecoveryAction({
      companyId,
      sourceIssueId,
    ...
    expect(patched.body.error).toContain("Entering blocked requires unresolved blockers");
    const [sourceIssue] = await db.select().from(issues).where(eq(issues.id, sourceIssueId));
