...
Общие локальные рабочие области повторно используют запись одной задачи только при совпадении физического каталога, Git-ветки, окружения и отпечатка конфигурации. Совпадения пути недостаточно. Новая запись вместо закрытой сохраняет `derivedFromExecutionWorkspaceId`.

Автоматическое архивирование общей записи проверяет все связанные задачи и их наследников, активные прогоны и сервисы. Новая привязка использует тот же транзакционный замок, что и архивирование. Удаление исходной задачи сохраняет сведения о её владельце-прогоне; возврат задачи в работу снимает уже закрытую привязку, чтобы следующий запуск проверил актуальное окружение. Общий каталог и незавершённые файлы сохраняются. Для изолированных копий продолжают действовать проверки сохранности и доставки изменений.
